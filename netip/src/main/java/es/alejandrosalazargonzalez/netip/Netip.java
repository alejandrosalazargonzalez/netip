...
public class Netip {
     ...
     */
    public static List<SubnetAllocation> allocateSubnets(long baseNetwork, long baseBroadcast,
            List<SubnetAllocation> reqList) {
        reqList.sort((a, b) -> b.hostsRequired - a.hostsRequired);
        List<SubnetAllocation> allocations = new ArrayList<>();

        long current = baseNetwork;
        for (SubnetAllocation req : reqList) {
            ...
            SubnetAllocation alloc = new SubnetAllocation(req.id, req.hostsRequired, reqPrefix, current);
     ...
     */
    public static void displayVlsmTable(List<SubnetAllocation> allocations) {
        ...
        for (SubnetAllocation alloc : allocations) {
     ...
     */
    public static void exportCsv(List<SubnetAllocation> allocations, String filename) {
    ...
    public static void main(String[] args) {
        ...
        if (!vlsmMode) {
            ...
            try (Scanner scanner = new Scanner(System.in)) {
                ...
                // contenedor temporal)
                List<SubnetAllocation> reqList = new ArrayList<>();
                for (int i = 1; i <= numSubnets; i++) {
                    ...
                    reqList.add(new SubnetAllocation(i, hostsReq, 0, 0));
                }

                List<SubnetAllocation> allocations = allocateSubnets(network, baseBroadcast, reqList);
